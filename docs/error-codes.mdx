---
title: "Error Codes"
description: "Understanding DNSTail API error responses and how to handle them"
---

## Overview

The DNSTail API uses conventional HTTP response codes to indicate the success or failure of an API request. Codes in the `2xx` range indicate success, codes in the `4xx` range indicate an error caused by the provided information, and codes in the `5xx` range indicate an error with DNSTail's servers.
...
<AccordionGroup>
  ...
  <Accordion title="402 - Payment Required" icon="credit-card">
    ...
    **How to fix:** Upgrade your plan in the [Dashboard](https://dashboard.dnstail.com) or remove existing monitors.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="500 - Internal Server Error" icon="server">
    An unexpected error occurred on DNSTail's servers.
    ...
    **How to fix:** This is a server-side issue. Retry your request after a brief wait. If the problem persists, contact [support@dnstail.com](mailto:support@dnstail.com).
  </Accordion>
...
<Tip>
  If you encounter an error not listed here or need assistance, contact us at [support@dnstail.com](mailto:support@dnstail.com) with the request ID from the response headers.
</Tip>